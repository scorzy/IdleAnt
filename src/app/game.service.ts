...
export class GameService {
      ...
      kongregateAPI.loadAPI(() => {
        ...
        setTimeout(() => {
          try {
            ...
            Observable.interval(this.kongFreq).subscribe(
              this.sendKong.bind(this)
            );
  ...
  checkUpgrades() {
    if (this.isMainNav) this.game.reloadUpIcons();
  }
  ...
  load(notify = true, first = false): number {
    try {
      if (typeof Storage !== "undefined") {
        let saveRaw = localStorage.getItem("save1");
        if (!saveRaw || saveRaw === "") saveRaw = localStorage.getItem("save");
        if (saveRaw) {
          ...
          if (last) {
            if (notify)
              this.toastr.success(
                "Idle time: " + moment.duration(Date.now() - last).humanize(),
                "Game Loaded"
              );
            this.last = last;
            return last;
          ...
          if (!first) this.toastr.error("No savegame found", "Error");
        }
      } else {
  ...
  sendKong() {
    try {
      this.kongregate.stats.submit(
        "Prestige",
        this.nonInfinite(this.game.maxLevel)
      );
      this.kongregate.stats.submit(
        "Prestige2",
        this.nonInfinite(this.game.maxLevel)
      ...
      this.kongregate.stats.submit(
        "World Done",
        this.nonInfinite(this.game.prestigeDone)
      );
  ...
  setSize() {
    if (this.game.options.width > 1050 && this.game.options.height > 700)
      this.kongregate.services.resizeGame(
